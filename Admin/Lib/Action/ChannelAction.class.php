...
public function insert(){
	    ...
	    $this->assign ( 'jumpUrl', Cookie::get ( '_currentUrl_' ) );
		if(!empty($_POST['reid'])){
		 	$tmp = 	M ("Channeltype")->where("id=".$_POST['reid'])->field("topid")->find();
		 	$topid = empty($tmp['topid'])?$_POST['reid']:$tmp['topid'];
		}
...
public function fmanage_insert(){
	    ...
	    if(!empty($_POST["isrel"])){
		    $tmp = $model->where("chid=".$this->_param("chid")." and isrel=1")->find();
		    if($tmp){
				$this->error('添加失败！该模型中已经存在一个字段关联显示');
		    }
	    }
		...
		if(!$model->create()) {
			...
			if(!empty($_POST['addon_relation'])){
			    list($a1, $a2) = explode('|', $_POST['addon_relation']);
			    $model->rel_table = $a1;
			    $model->rel_field = $a2;
			}
